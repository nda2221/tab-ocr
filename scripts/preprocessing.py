...
def detect(cropped_frame, is_number = False, lang = 'rus'):
    if (is_number):
        text = pytesseract.image_to_string(cropped_frame, config ='-l ' + lang + ' --psm 6')
    else:
        text = pytesseract.image_to_string(cropped_frame, config = '-l ' + lang + '--psm 4')
